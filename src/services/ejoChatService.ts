...
export const ejoChatService = {
  async send(providerId: string, messages: ChatMessage[], conversationId?: string | null, regenerate = false, mode: 'chat' | 'do' = 'chat'): Promise<EjoChatReply | null> {
    const res = await fetch('/api/chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ providerId, conversationId, regenerate, mode, messages }),
    });
    if (res.status === 429) {
